...
public class Timer
{
    ...
    private MonoBehaviour currentUsingObject;
    ...
    public void Cancel()
    {
        if (IsDone) return;
        IsDone = true;
        currentUsingObject.StopCoroutine(currentCoroutine);
    }
    ...
    public void StartTimer(float time, Action OnCountDownOver, MonoBehaviour usingObject)
    {
        ...
        currentCoroutine = usingObject.StartCoroutine(CountDown(time, OnCountDownOver));
    }

    /// <summary>
    /// Coroutine that counts down and executes an action when the time is over
    /// </summary>
    /// <param name="time"time until the action is executed></param>
    /// <param name="OnCountDownOver">Action to be executed</param>
    /// <returns></returns>
